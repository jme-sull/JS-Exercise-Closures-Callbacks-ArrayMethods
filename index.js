...
function processContains(item, list, callback) {
  ...
  let result = callback(list.includes(item));
    return result;  
...
function getFullNames(runners) {
  ...
  function getNames(runners){
    nameArray.push(`${runners.last_name}, ${runners.first_name}`);
  }
...
function firstNamesAllCaps(runners) {
  ...
  const firstNames = runners.map((first_name) => {
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  ...
  const filterShirtSize = runners.filter((shirt_size) => {
    return shirt_size.shirt_size === tShirtSize;});

  return filterShirtSize;
...
function tallyUpDonations(runners) {
  const totalDonations = runners.reduce((total, donation) => {
